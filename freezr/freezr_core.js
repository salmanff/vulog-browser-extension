...
freezr.feps.postquery = function (...optionsAndCallback) {
  ...
  if (options.sort) writeOptions.sort = options.sort // 2024-04 => need to add to main freezr_core
...
freezerRestricted.connect.send = function (url, postData, callback, method, contentType, options = {}) {
  ...
  if (!options.appToken && !freezrMeta.appToken && !freezr.utils.getCookie('app_token_' + freezrMeta.userId) && PATHS_WO_TOKEN.indexOf(coreUrl) < 0) {
    ...
    async function sendReq (url, postData) { // https://developer.mozilla.org/en-US/docs/Web/API/Fetch_API/Using_Fetch
      ...
      // onsole.log('core resposne ', { response })
...
freezerRestricted.menu.resetDialogueBox = function (isAdminPage, addText) {
  ...
  if (innerText) innerText.innerHTML = (addText ? ('<br/><div>' + addText + '</div>') : '') + '<br/><div align="center">.<img src="' + (  ' ?? '  ) + '"/></div>' //deleted by mistake - to do to review
